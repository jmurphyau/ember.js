...
if (isEnabled('ember-htmlbars-get-helper')) {
  ...
  QUnit.test('should be able to get an object value with a bound/dynamic key JAMESTEST', function() {
    ...
    view = EmberView.create({
      context: context,
      template: compile('[{{get colors key}}]{{#if showSounds}}[{{get sounds soundKey}}][{{get colors key}}]{{/if}}')
    });

    runAppend(view);

    equal(view.$().text(), '[red]', 'should return \'red\' for {{get colors key}}  (key = \'apple\')');
    ...
    equal(view.$().text(), '[green]', 'should return \'green\' for {{get colors key}}  (key = \'apple\')');
    ...
    equal(view.$().text(), '[yellow]', 'should return \'red\' for {{get colors key}} (key = \'banana\')');
